...
image = cv2.imread('pytorch_D-RISE/images/mouse_keyboard.jpg')
...
target_box = np.array([289, 177, 682, 432])
saliency_map = generate_saliency_map(image,
                                     target_class_index=62,
...
plt.axis('off')
plt.savefig('Image42.png')

#########################################################################

target_box = np.array([550, 488, 619, 531])
saliency_map = generate_saliency_map(image,
                                     target_class_index=64,
                                     target_box=target_box,
                                     prob_thresh=0.5,
                                     grid_size=(16, 16),
                                     n_masks=1000)
...
target_box = np.array([237, 467, 507, 530])
saliency_map = generate_saliency_map(image,
                                     target_class_index=66,
                                     target_box=target_box,
                                     prob_thresh=0.5,
                                     grid_size=(16, 16),
                                     n_masks=1000)
